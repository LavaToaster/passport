...
class BridgeAccessTokenRepositoryTest extends PHPUnit_Framework_TestCase
{
    ...
    public function test_access_tokens_can_be_persisted()
    {
        ...
        $accessToken = new LaravelDoctrine\Passport\Bridge\AccessToken(2, [new LaravelDoctrine\Passport\Bridge\Scope('scopes')]);
        $accessToken->setIdentifier(1);
        $accessToken->setExpiryDateTime($expiration);
        $accessToken->setClient(new LaravelDoctrine\Passport\Bridge\Client('client-id', 'name', 'redirect'));

        $repository = new LaravelDoctrine\Passport\Bridge\AccessTokenRepository($database, $events);
