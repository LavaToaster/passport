...
use LaravelDoctrine\Passport\Guards\TokenGuard;
use Illuminate\Support\ServiceProvider;
use LaravelDoctrine\Passport\Repositories\AuthCodeRepository;
...
use LaravelDoctrine\Passport\Bridge\PersonalAccessGrant;
    ...
    protected function registerEntities()
    {
        $this->app->afterResolving('registry', function (IlluminateRegistry $registry, Container $container) {
            ...
            foreach ($registry->getManagers() as $manager) {
                /** @var \LaravelDoctrine\ORM\Extensions\MappingDriverChain $metadataDriver */
                ...
                $metadataDriver->addPaths([__DIR__.'/Entities']);
            }
        });

        $repositories = [
            AuthCodeRepository::class => AuthCode::class,
            ClientRepository::class => Client::class,
            DoctrineRefreshTokenRepository::class => RefreshToken::class,
            TokenRepository::class => Token::class
        ];

        foreach ($repositories as $repository => $entity) {
            $this->app->bind($repository, function (Application $app) use ($repository, $entity) {
                return new $repository($app->make('em'), new ClassMetadata($entity));
            });
        }
    }
